...
public class ProductController {
    ...
    public ResponseEntity<ServerResponse<?>> getProducts(
            @Parameter(name = "상품 가시성 여부", description = "ALL - visible 여부 관계 없이 리턴, TRUE-true인 것만, FALSE - false인 것만")
    ...
    @Operation(summary = "상품 세트 관련 상품 조회 API", description = "상품 세트 관련 상품 조회 API입니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공시", content = @Content(schema = @Schema(implementation = SimpleProductInfo.class)))
    @FailApiResponses
    @GetMapping("/bundle")
    public ResponseEntity<ServerResponse<?>> getCurrentMarketProducts(
